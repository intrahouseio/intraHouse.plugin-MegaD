...
function start(plugin, logger) {
  ...
  function onRequest(request, response) {
    ...
    // Системный запрос st=1 - сформировать список таймеров для опроса с нуля
    if (qobj.st == 1) {
        
      plugin.formTimers();
      
      // Передать текущее время - опционально   
      if (plugin.params.stCurrentTime) plugin.addActReq(ut.formCurrentTime());
    ...
    if (mreqobj && mreqobj.fullurl) sendFullUrl(mreqobj.fullurl); 
  }
