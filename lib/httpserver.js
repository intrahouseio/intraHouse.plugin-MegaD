...
function start(plugin, logger) {
  ...
  function onRequest(request, response) {
    ...
    if (mreqobj) {
      ...
      if (mreqobj.startscene) {
        ...
        logger.log(
          util.inspect(
            { type: 'startscene', id: mreqobj.startscene, arg: mreqobj.arg || JSON.stringify(qobj) },
            'server'
          )
        );
        process.send({ type: 'startscene', id: mreqobj.startscene, arg: mreqobj.arg || JSON.stringify(qobj) });
      }
    }
  }
    ...
    if (tableMReq && pathname && query && tableMReq[pathname]) {
      ...
      if (tableMReq[pathname][id] && util.isArray(tableMReq[pathname][id])) {
        for (var i = 0; i < tableMReq[pathname][id].length; i++) {
          if (tableMReq[pathname][id][i].mcp) {
            if (isMcpSuitable(tableMReq[pathname][id][i].queryprops, query)) return tableMReq[pathname][id][i];
          } else if (isSuitable(tableMReq[pathname][id][i].queryprops, query)) {
  ...
  function isMcpSuitable(patobj, qobj) {
    if (!patobj || !qobj) return;
    let count = 0;
    for (var prop in patobj) {
      if (patobj[prop] == '*') {
        if (qobj[prop] != undefined) count++;
      } else if (qobj[prop] != patobj[prop]) return;
    }
    return count > 0;
  }
  ...
  function formTableMReq(reqarr) {
    ...
    function getOneReqObj(item) {
      ...
      oneobj.q = typeof oneobj.queryprops == 'object' ? Object.keys(oneobj.queryprops).length : 0;
      return oneobj;
    }
  }
}
